...
const AUTHENTICATED_NAV_ITEMS = {
  ...
  worker: [["home", "Home", "⌂"], ["work", "Work", "◫"]], trainer: [["home", "Home", "⌂"], ["work", "Work", "◫"]], management: [["home", "Home", "⌂"], ["work", "Work", "◫"]]
};
...
function renderShell(content) {
    ...
    <div class="shell ${authenticatedWorkspace ? "authenticated-shell" : ""} ${authenticatedContext ? "database-workspace" : ""}">
      <header class="topbar">
        ...
        <div class="top-actions">
          ${authenticatedWorkspace ? `<button class="notification-button" id="notificationButton" aria-label="Notifications"><span aria-hidden="true">●</span></button>` : ""}
  ...
  document.getElementById("notificationButton")?.addEventListener("click", () => {
    ...
    state.activeWorkspaceView = "work";
    saveState();
    renderAuthenticatedWorkspace();
  });
...
function renderAuthenticatedWorkspace() {
  ...
  const trainerRelationships = c.trainerAssignments.filter(item => !selected || item.department_id === selected.id);
  const phase3Assignments = c.learningAssignments || [];
  const phase3Cards = phase3Assignments.map(assignment => {
    const pathway=(c.learningPathways||[]).find(item=>item.id===assignment.pathway_id), version=(c.learningPathwayVersions||[]).find(item=>item.id===assignment.pathway_version_id);
    const progress=(c.learningItemProgress||[]).filter(item=>item.assignment_id===assignment.id);
    const items=(c.learningModuleItems||[]).filter(item=>item.pathway_version_id===assignment.pathway_version_id);
    const workerItems=learner?items.map(item=>{const row=progress.find(p=>p.item_id===item.id),done=row?.status==="Completed";if(item.item_type==="Quiz"){const q=item.content?.questions?.[0]||{};return `<article class="phase3-item"><strong>${escapeHtml(item.title)}</strong><p>${escapeHtml(q.prompt||"Knowledge check")}</p><select class="phase3-quiz-answer" data-item="${escapeHtml(item.id)}">${(q.options||[]).map((option,index)=>`<option value="${index}">${escapeHtml(option)}</option>`).join("")}</select><button class="btn complete-phase3-item" data-assignment="${escapeHtml(assignment.id)}" data-item="${escapeHtml(item.id)}" ${done?"disabled":""}>${done?`Passed · ${row.score}%`:"Submit knowledge check"}</button></article>`}return `<article class="phase3-item"><strong>${escapeHtml(item.title)}</strong><p>${escapeHtml(item.content?.body||item.content?.description||"Required learning activity")}</p><button class="btn complete-phase3-item" data-assignment="${escapeHtml(assignment.id)}" data-item="${escapeHtml(item.id)}" ${done?"disabled":""}>${done?"Completed":"Mark complete"}</button></article>`}).join(""):"";
    const rubric=(c.competencyRubrics||[]).find(item=>item.pathway_version_id===assignment.pathway_version_id&&item.status==="Published"), assessment=(c.competencyAssessments||[]).find(item=>item.assignment_id===assignment.id&&!['Approved','Reassessment Required'].includes(item.status));
    const criteria=rubric?(c.competencyRubricCriteria||[]).filter(item=>item.rubric_id===rubric.id):[];
    const criterionForm=trainer&&assessment?.status==="Draft"?`<section class="phase4-assessment" data-assessment="${escapeHtml(assessment.id)}"><span class="eyebrow">CRITERION-LEVEL ASSESSMENT</span>${criteria.map((criterion,index)=>`<article class="phase4-criterion" data-criterion="${escapeHtml(criterion.id)}"><div><strong>${index+1}. ${escapeHtml(criterion.criterion)}</strong>${criterion.safety_critical?'<span class="status-chip status-warning">Safety critical</span>':""}</div><select class="phase4-rating"><option>Competent</option><option>Needs Development</option><option>Not Observed</option></select><textarea class="phase4-comments" placeholder="${criterion.comments_required?'Required criterion comments':'Criterion comments'}"></textarea></article>`).join("")}<label>Assessment location<input class="phase4-location" placeholder="Work area or department"></label><label>Context<textarea class="phase4-context" placeholder="Date, task and workplace context"></textarea></label><label>Development plan<textarea class="phase4-development" placeholder="Required when development or reassessment is needed"></textarea></label><label class="checkbox-label"><input class="phase4-observed" type="checkbox"> I personally observed this practical assessment</label><button class="btn submit-phase4-assessment" data-assessment="${escapeHtml(assessment.id)}">Submit evidence package</button></section>`:"";
    // Phase 4 replaces the legacy “Send recommendation” control with a complete criterion evidence package.
    const trainerActions=trainer?(assessment?criterionForm:`<button class="btn start-phase4-assessment" data-assignment="${escapeHtml(assignment.id)}" ${!rubric||assignment.status!=="Ready for Trainer"?"disabled":""}>${rubric?"Start practical assessment":"Published rubric required"}</button>`):"";
    const acknowledgement=learner&&assessment?.status==="Submitted"?`<section class="phase4-acknowledgement"><strong>Worker acknowledgement</strong><p>Review the recorded criterion results. Your comment does not change the assessor decision.</p><textarea class="phase4-worker-comment" placeholder="Optional worker comment"></textarea><button class="btn acknowledge-phase4" data-assessment="${escapeHtml(assessment.id)}">Acknowledge assessment</button></section>`:"";
    return `<article class="card phase3-learning-card"><span class="eyebrow">VERSION ${escapeHtml(version?.version_number||"")} · ${escapeHtml(assignment.status)}</span><h3>${escapeHtml(pathway?.title||"Assigned pathway")}</h3><div class="area-progress"><span style="width:${Number(assignment.progress_percent)||0}%"></span></div><small>${Number(assignment.progress_percent)||0}% complete</small>${workerItems}${trainerActions}${acknowledgement}<p class="auth-status" id="phase3-status-${escapeHtml(assignment.id)}"></p></article>`;
  }).join("");
  ...
  const summary = learner
    ? `<div class="stats-grid"><div class="stat-card"><span>Assigned pathways</span><strong>${phase3Assignments.length||assignments.length}</strong></div><div class="stat-card"><span>Unread notifications</span><strong>${c.notifications.length}</strong></div><div class="stat-card"><span>Competency records</span><strong>${(c.competencyAwards||[]).length||c.competencyRecords.length}</strong></div></div><div class="section-heading" id="training"><div><span class="eyebrow">YOUR TRAINING</span><h3>Assigned pathways</h3></div></div><div class="grid grid-2">${phase3Cards||assignmentCards||'<p class="empty-state">No pathway has been assigned in this workspace.</p>'}</div>`
    : trainer
      ? `<div class="stats-grid"><div class="stat-card"><span>Assigned trainees</span><strong>${phase3Assignments.length||trainerRelationships.length}</strong></div><div class="stat-card"><span>Pending reviews</span><strong>${phase3Assignments.filter(item=>item.status==="Ready for Trainer").length}</strong></div><div class="stat-card"><span>Unread notifications</span><strong>${c.notifications.length}</strong></div></div><div class="section-heading" id="staff"><div><span class="eyebrow">ASSIGNED TRAINEES</span><h3>Training and competency</h3></div></div><div class="grid grid-2">${phase3Cards||traineeRows||'<p class="empty-state">No trainees are assigned in this department.</p>'}</div>`
  ...
  document.querySelectorAll(".complete-phase3-item").forEach(button=>button.addEventListener("click",async()=>{const status=document.getElementById(`phase3-status-${button.dataset.assignment}`),select=document.querySelector(`.phase3-quiz-answer[data-item="${button.dataset.item}"]`);button.disabled=true;status.textContent="Saving verified progress…";try{await authService.database.completeLearningItem(button.dataset.assignment,button.dataset.item,select?{selectedOption:Number(select.value)}:{});authenticatedContext=await authService.restore(state.activeOrganizationId);renderAuthenticatedWorkspace();}catch{status.textContent="This activity could not be completed.";button.disabled=false;}}));
  document.querySelectorAll(".record-phase3-observation").forEach(button=>button.addEventListener("click",async()=>{const id=button.dataset.assignment,note=document.querySelector(`.phase3-observation-note[data-assignment="${id}"]`).value.trim(),outcome=document.querySelector(`.phase3-observation-outcome[data-assignment="${id}"]`).value,status=document.getElementById(`phase3-status-${id}`);if(note.length<10)return status.textContent="Record at least 10 characters of observable evidence.";try{await authService.database.recordCompetencyObservation(id,outcome,note);authenticatedContext=await authService.restore(state.activeOrganizationId);renderAuthenticatedWorkspace();}catch{status.textContent="Observation was not authorised.";}}));
  document.querySelectorAll(".submit-phase3-recommendation").forEach(button=>button.addEventListener("click",async()=>{const id=button.dataset.assignment,note=document.querySelector(`.phase3-observation-note[data-assignment="${id}"]`).value.trim(),outcome=document.querySelector(`.phase3-observation-outcome[data-assignment="${id}"]`).value,status=document.getElementById(`phase3-status-${id}`);try{await authService.database.submitCompetencyRecommendation(id,outcome,note);authenticatedContext=await authService.restore(state.activeOrganizationId);renderAuthenticatedWorkspace();}catch{status.textContent="Record an authorised observation before recommending competency.";}}));
  document.querySelectorAll(".start-phase4-assessment").forEach(button=>button.addEventListener("click",async()=>{try{await authService.database.startCompetencyAssessment(button.dataset.assignment);authenticatedContext=await authService.restore(state.activeOrganizationId);renderAuthenticatedWorkspace();}catch{document.getElementById(`phase3-status-${button.dataset.assignment}`).textContent="A published rubric and trainer-ready assignment are required.";}}));
  document.querySelectorAll(".submit-phase4-assessment").forEach(button=>button.addEventListener("click",async()=>{const section=button.closest(".phase4-assessment"),status=button.closest(".phase3-learning-card").querySelector(".auth-status");button.disabled=true;status.textContent="Validating every criterion…";try{for(const row of section.querySelectorAll(".phase4-criterion"))await authService.database.saveCompetencyCriterion(section.dataset.assessment,row.dataset.criterion,row.querySelector(".phase4-rating").value,row.querySelector(".phase4-comments").value||null);await authService.database.submitCompetencyAssessment({assessmentId:section.dataset.assessment,location:section.querySelector(".phase4-location").value,context:section.querySelector(".phase4-context").value,developmentPlan:section.querySelector(".phase4-development").value,personallyObserved:section.querySelector(".phase4-observed").checked});authenticatedContext=await authService.restore(state.activeOrganizationId);renderAuthenticatedWorkspace();}catch{status.textContent="Complete every criterion, required comment, evidence declaration and development plan.";button.disabled=false;}}));
  document.querySelectorAll(".acknowledge-phase4").forEach(button=>button.addEventListener("click",async()=>{try{await authService.database.acknowledgeCompetencyAssessment(button.dataset.assessment,true,button.closest(".phase4-acknowledgement").querySelector(".phase4-worker-comment").value||null);authenticatedContext=await authService.restore(state.activeOrganizationId);renderAuthenticatedWorkspace();}catch{button.closest(".phase3-learning-card").querySelector(".auth-status").textContent="This acknowledgement could not be recorded.";}}));
}
...
function renderEducatorWorkspace(context) {
  ...
  const workspaceHero = (eyebrow, title, description) => `${organizationSwitcher(context)}<section class="dashboard-hero workspace-page-hero"><div class="dashboard-welcome"><span class="eyebrow">${eyebrow}</span><h2>${title}</h2><p>${description}</p></div></section>`;
...
function pathwayLifecycleTone(lifecycle) {
  if (lifecycle === "Published") return "success";
  if (["In Review", "Approved"].includes(lifecycle)) return "warning";
  return "neutral";
}

function pathwayAuthoringHtml(context, workspaceHero) {
  const pathways = context.learningPathways || [], versions = context.learningPathwayVersions || [];
  const modules = context.learningModules || [], items = context.learningModuleItems || [];
  const selectedPathway = pathways.find(item => item.id === state.selectedLearningPathwayId) || pathways[0] || null;
  if (selectedPathway) state.selectedLearningPathwayId = selectedPathway.id;
  const pathwayVersions = selectedPathway ? versions.filter(item => item.pathway_id === selectedPathway.id) : [];
  const selectedVersion = pathwayVersions.find(item => item.id === state.selectedLearningVersionId)
    || pathwayVersions.find(item => item.lifecycle === "Draft")
    || pathwayVersions.find(item => item.id === selectedPathway?.current_version_id)
    || pathwayVersions[0] || null;
  if (selectedVersion) state.selectedLearningVersionId = selectedVersion.id;
  const versionModules = selectedVersion ? modules.filter(item => item.pathway_version_id === selectedVersion.id) : [];
  const editable = selectedVersion?.lifecycle === "Draft";
  const canApprove = context.membership?.role === "Organisation Administrator";
  const pathwayList = pathways.map(pathway => {
    const current = versions.find(version => version.id === pathway.current_version_id);
    return `<button class="pathway-library-row ${selectedPathway?.id === pathway.id ? "is-active" : ""}" data-learning-pathway="${escapeHtml(pathway.id)}"><span>▷</span><div><strong>${escapeHtml(pathway.title)}</strong><small>${escapeHtml(pathway.code)} · ${escapeHtml(pathway.sector)}</small></div><span class="status-chip status-${pathwayLifecycleTone(current?.lifecycle || "Draft")}">${escapeHtml(current?.lifecycle || "Draft")}</span></button>`;
  }).join("");
  const versionTabs = pathwayVersions.map(version => `<button class="version-tab ${version.id === selectedVersion?.id ? "is-active" : ""}" data-learning-version="${escapeHtml(version.id)}"><strong>v${version.version_number}</strong><span>${escapeHtml(version.lifecycle)}</span></button>`).join("");
  const moduleCards = versionModules.map(module => {
    const moduleItems = items.filter(item => item.module_id === module.id);
    return `<article class="card authoring-module"><div class="section-heading"><div><span class="eyebrow">MODULE ${module.position + 1}</span><h3>${escapeHtml(module.title)}</h3><p>${escapeHtml(module.description || "Required learning module")}</p></div><span class="count-badge">${moduleItems.length}</span></div><div class="module-item-list">${moduleItems.map(item => `<article class="module-item-row"><span class="item-type-icon">${item.item_type === "Video" ? "▶" : item.item_type === "Quiz" ? "?" : item.item_type === "Page" ? "≡" : "⇩"}</span><div><strong>${escapeHtml(item.title)}</strong><small>${escapeHtml(item.item_type)} · ${escapeHtml(item.completion_requirement)}</small></div><span class="status-chip status-neutral">${item.is_required ? "Required" : "Optional"}</span></article>`).join("") || '<p class="empty-state">No lessons or assessments in this module yet.</p>'}</div>${editable ? `<form class="authoring-item-form" data-module-id="${escapeHtml(module.id)}"><div class="form-grid"><label>Item type<select name="type"><option>Page</option><option>Video</option><option>File</option><option>Downloadable Resource</option><option>External Link</option><option>Quiz</option></select></label><label>Title<input name="title" maxlength="200" required placeholder="Lesson or assessment title"></label></div><label>Lesson text<textarea name="body" placeholder="Instructions, procedure or lesson content"></textarea></label><label>Secure resource URL<input name="url" type="url" placeholder="https://…"></label><fieldset class="quiz-fields"><legend>Quiz question</legend><label>Question<input name="question" placeholder="What should the learner know?"></label><label>Options, one per line<textarea name="options" placeholder="First option&#10;Second option"></textarea></label><div class="form-grid"><label>Correct option number<input name="correctOption" type="number" min="1" value="1"></label><label>Pass mark %<input name="passMark" type="number" min="1" max="100" value="80"></label></div></fieldset><label class="checkbox-label"><input name="required" type="checkbox" checked> Required item</label><button class="btn btn-secondary" type="submit">Add to module</button><p class="auth-status" role="status"></p></form>` : ""}</article>`;
  }).join("");
  const lifecycleActions = !selectedVersion ? "" : `<div class="authoring-actions">${editable ? '<button class="btn content-transition" data-action="submit">Submit for review</button>' : ""}${selectedVersion.lifecycle === "In Review" && canApprove ? '<button class="btn content-transition" data-action="approve">Approve version</button>' : ""}${selectedVersion.lifecycle === "Approved" && canApprove ? '<button class="btn content-transition" data-action="publish">Publish version</button>' : ""}${selectedVersion.lifecycle === "Published" && !pathwayVersions.some(item => item.lifecycle === "Draft") ? '<button class="btn btn-secondary" id="createPathwayVersion">Create new version</button>' : ""}<p class="auth-status" id="pathwayLifecycleStatus" role="status"></p></div>`;
  const builder = !selectedPathway ? `<section class="card purpose-empty"><span>＋</span><h3>Create the first pathway</h3><p>Use the form to establish a controlled, organisation-owned training pathway.</p></section>` : `<section class="pathway-builder-main"><section class="card pathway-version-header"><div><span class="eyebrow">${escapeHtml(selectedPathway.code)} · VERSION CONTROL</span><h3>${escapeHtml(selectedPathway.title)}</h3><p>${escapeHtml(selectedVersion?.description || selectedPathway.summary || "Organisation training pathway")}</p></div><span class="status-chip status-${pathwayLifecycleTone(selectedVersion?.lifecycle)}">${escapeHtml(selectedVersion?.lifecycle || "Draft")}</span></section><nav class="version-tabs" aria-label="Pathway versions">${versionTabs}</nav>${lifecycleActions}<div class="authoring-module-list">${moduleCards || '<section class="card purpose-empty compact"><h4>No modules yet</h4><p>Add the first module to begin structuring this pathway.</p></section>'}</div>${editable ? `<form class="card setup-form" id="learningModuleForm"><span class="eyebrow">ADD MODULE</span><div class="form-grid"><label>Module title<input name="title" maxlength="200" required></label><label>Description<input name="description"></label></div><label class="checkbox-label"><input name="sequential" type="checkbox"> Require sequential completion</label><button class="btn" type="submit">Add module</button><p class="auth-status" role="status"></p></form>` : '<section class="card locked-content-note"><strong>Published content is immutable</strong><p>Create a new version to make controlled changes without altering historical completions.</p></section>'}</section>`;
  return `${workspaceHero("PATHWAY AUTHORING", "Content and pathway builder", "Create modules, lessons, videos, resources and quizzes, then move each immutable version through review and publication.")}<section class="pathway-authoring-layout"><aside class="card pathway-library"><div class="section-heading"><div><span class="eyebrow">LIBRARY</span><h3>${pathways.length} pathways</h3></div></div><div class="pathway-library-list">${pathwayList || '<p class="empty-state">No pathways yet.</p>'}</div><form class="pathway-create-form" id="learningPathwayForm"><span class="eyebrow">NEW PATHWAY</span><div class="form-grid"><label>Code<input name="code" pattern="[A-Za-z0-9][A-Za-z0-9_-]{1,39}" maxlength="40" required placeholder="PCA-OT"></label><label>Renewal days<input name="renewalDays" type="number" min="1" max="3650"></label></div><label>Title<input name="title" maxlength="200" required></label><label>Summary<textarea name="summary"></textarea></label><label>Learning objectives, one per line<textarea name="objectives" placeholder="Move patients safely&#10;Prepare equipment correctly"></textarea></label><button class="btn" type="submit">Create draft pathway</button><p class="auth-status" role="status"></p></form></aside>${builder}</section>`;
}

function bindPathwayAuthoring(context) {
  document.querySelectorAll("[data-learning-pathway]").forEach(button => button.addEventListener("click", () => { state.selectedLearningPathwayId = button.dataset.learningPathway; state.selectedLearningVersionId = null; saveState(); renderAuthenticatedWorkspace(); }));
  document.querySelectorAll("[data-learning-version]").forEach(button => button.addEventListener("click", () => { state.selectedLearningVersionId = button.dataset.learningVersion; saveState(); renderAuthenticatedWorkspace(); }));
  bindSetupForm("learningPathwayForm", async values => { const created = await authService.database.createLearningPathway(context, { code:values.get("code"), title:values.get("title"), summary:values.get("summary"), objectives:String(values.get("objectives") || "").split("\n").map(item => item.trim()).filter(Boolean), renewalDays:Number(values.get("renewalDays")) || null }); state.selectedLearningPathwayId = created.pathway_id; state.selectedLearningVersionId = created.version_id; });
  bindSetupForm("learningModuleForm", values => authService.database.addLearningModule({ versionId:state.selectedLearningVersionId, title:values.get("title"), description:values.get("description"), sequential:values.get("sequential") === "on" }));
  document.querySelectorAll(".authoring-item-form").forEach(form => form.addEventListener("submit", async event => {
    event.preventDefault(); const status = form.querySelector(".auth-status"), values = new FormData(form), type = values.get("type"); status.textContent = "Adding item…";
    const options = String(values.get("options") || "").split("\n").map(item => item.trim()).filter(Boolean);
    const content = type === "Page" ? { body:String(values.get("body") || "").trim() }
      : type === "Quiz" ? { questions:[{ prompt:String(values.get("question") || "").trim(), options, correctOption:Math.max(0, Number(values.get("correctOption")) - 1) }] }
        : { url:String(values.get("url") || "").trim() };
    const completion = type === "Quiz" ? "Minimum Score" : type === "Page" ? "Mark Complete" : "View";
    try { await authService.database.addLearningModuleItem({ moduleId:form.dataset.moduleId, type, title:values.get("title"), completion, content, configuration:type === "Quiz" ? { passMark:Number(values.get("passMark")) } : {}, required:values.get("required") === "on" }); authenticatedContext = await authService.restore(state.activeOrganizationId); renderAuthenticatedWorkspace(); }
    catch { status.textContent = "The item could not be added. Check the required content and secure URL."; }
  }));
  document.querySelectorAll(".content-transition").forEach(button => button.addEventListener("click", async () => { const status=document.getElementById("pathwayLifecycleStatus"); if (button.dataset.action === "publish" && !confirm("Publish this version? Published content becomes immutable and visible to authorised learners.")) return; button.disabled=true; status.textContent="Updating controlled lifecycle…"; try { await authService.database.transitionLearningPathwayVersion(state.selectedLearningVersionId, button.dataset.action); authenticatedContext=await authService.restore(state.activeOrganizationId); renderAuthenticatedWorkspace(); } catch { status.textContent="This lifecycle change is not authorised or the pathway is incomplete."; button.disabled=false; } }));
  document.getElementById("createPathwayVersion")?.addEventListener("click", async () => { const status=document.getElementById("pathwayLifecycleStatus"); if(!confirm("Create a new editable version from the published pathway?")) return; try { state.selectedLearningVersionId=await authService.database.createLearningPathwayVersion(state.selectedLearningPathwayId); authenticatedContext=await authService.restore(state.activeOrganizationId); saveState(); renderAuthenticatedWorkspace(); } catch { status.textContent="A new version could not be created."; } });
}

function phase3CompetencyHtml(context, workspaceHero) {
  // Assessment and assurance remains the organisation competency destination.
  const assignments=context.learningAssignments||[], awards=context.competencyAwards||[], observations=context.competencyObservations||[], recommendations=context.competencyRecommendations||[];
  const published=(context.learningPathwayVersions||[]).filter(item=>item.lifecycle==="Published");
  const profiles=new Map((context.organizationStaff||[]).map(item=>[item.user_id,item.user_profiles||{}]));
  const memberships=context.organizationMemberships||[];
  const workers=memberships.filter(item=>["PCA","Cleaner","Support Worker"].includes(item.role));
  const trainers=memberships.filter(item=>item.role?.includes("Trainer"));
  const rubrics=context.competencyRubrics||[], sections=context.competencyRubricSections||[], criteria=context.competencyRubricCriteria||[], assessments=context.competencyAssessments||[], reviews=context.competencyManagementReviews||[];
  const assignmentRows=assignments.map(item=>{const worker=profiles.get(item.worker_user_id)||{};const version=published.find(v=>v.id===item.pathway_version_id);const pathway=(context.learningPathways||[]).find(p=>p.id===item.pathway_id);const observation=observations.find(o=>o.assignment_id===item.id);const recommendation=recommendations.find(o=>o.assignment_id===item.id);const award=awards.find(o=>o.assignment_id===item.id);const assessment=assessments.find(a=>a.assignment_id===item.id),review=assessment&&reviews.find(r=>r.assessment_id===assessment.id);return `<article class="phase3-assignment" data-assignment-id="${escapeHtml(item.id)}"><div><span class="eyebrow">${escapeHtml(item.status)}</span><h3>${escapeHtml(worker.full_name||"Assigned worker")}</h3><p>${escapeHtml(pathway?.title||"Published pathway")} · Version ${escapeHtml(version?.version_number||"")}</p><div class="area-progress"><span style="width:${Number(item.progress_percent)||0}%"></span></div><small>${Number(item.progress_percent)||0}% learning complete${item.due_at?` · Due ${escapeHtml(new Date(item.due_at).toLocaleDateString("en-AU"))}`:""}</small></div><div class="phase3-evidence"><span>Criterion assessment: <strong>${assessment?escapeHtml(assessment.status):"Pending"}</strong></span><span>Overall rating: <strong>${assessment?.overall_rating?escapeHtml(assessment.overall_rating):"Pending"}</strong></span><span>Management review: <strong>${review?escapeHtml(review.decision):"Pending"}</strong></span>${assessment?.status==="Management Review"?`<label>Review reason<textarea class="phase4-review-reason" placeholder="Record the evidence-based reason"></textarea></label><label>Validity days<input class="phase4-validity" type="number" min="1" max="3650" value="365"></label><div><button class="btn phase4-review" data-assessment="${escapeHtml(assessment.id)}" data-decision="Approve">Approve evidence</button><button class="btn btn-secondary phase4-review" data-assessment="${escapeHtml(assessment.id)}" data-decision="Request Reassessment">Request reassessment</button></div>`:""}${!assessment?`<span>Legacy observation: <strong>${observation?escapeHtml(observation.outcome):"Pending"}</strong></span><span>Legacy recommendation: <strong>${recommendation?escapeHtml(recommendation.recommendation):"Pending"}</strong></span><span>Legacy decision: <strong>${award?escapeHtml(award.decision):"Pending"}</strong></span>`:""}</div></article>`}).join("");
  const form=published.length&&workers.length?`<form class="card setup-form" id="phase3AssignmentForm"><span class="eyebrow">ASSIGN PUBLISHED LEARNING</span><h3>New assignment</h3><label>Published version<select name="versionId">${published.map(v=>{const p=(context.learningPathways||[]).find(x=>x.id===v.pathway_id);return `<option value="${escapeHtml(v.id)}">${escapeHtml(p?.title||"Pathway")} · Version ${v.version_number}</option>`}).join("")}</select></label><label>Worker<select name="workerUserId">${workers.map(m=>`<option value="${escapeHtml(m.user_id)}">${escapeHtml(profiles.get(m.user_id)?.full_name||m.role)}</option>`).join("")}</select></label><label>Trainer<select name="trainerUserId"><option value="">Assign later</option>${trainers.map(m=>`<option value="${escapeHtml(m.user_id)}">${escapeHtml(profiles.get(m.user_id)?.full_name||m.role)}</option>`).join("")}</select></label><label>Due date<input type="date" name="dueAt"></label><button class="btn" type="submit">Assign pathway</button><p class="auth-status" role="status"></p></form>`:`<section class="card purpose-empty"><h3>Assignment setup required</h3><p>Publish a pathway and invite at least one worker before creating a production assignment.</p></section>`;
  const rubricCards=rubrics.map(rubric=>{const rubricSections=sections.filter(s=>s.rubric_id===rubric.id);const rubricCriteria=criteria.filter(c=>c.rubric_id===rubric.id);return `<article class="card phase4-rubric" data-rubric="${escapeHtml(rubric.id)}"><div class="section-heading"><div><span class="eyebrow">RUBRIC V${rubric.version_number}</span><h3>${escapeHtml(rubric.title)}</h3></div><span class="status-chip status-${rubric.status==="Published"?"success":"warning"}">${escapeHtml(rubric.status)}</span></div><p>${escapeHtml(rubric.assessor_guidance||"Criterion-level practical assessment")}</p>${rubricSections.map(section=>`<section><strong>${escapeHtml(section.title)}</strong><ul>${rubricCriteria.filter(c=>c.section_id===section.id).map(c=>`<li>${c.safety_critical?"⚠ ":""}${escapeHtml(c.criterion)}${c.evidence_required?" · Evidence required":""}</li>`).join("")}</ul></section>`).join("")}${rubric.status==="Draft"?`<form class="phase4-section-form"><label>Section title<input name="title" required></label><button class="btn btn-secondary">Add section</button></form>${rubricSections.map(section=>`<form class="phase4-criterion-form" data-section="${escapeHtml(section.id)}"><label>Criterion<input name="criterion" required></label><div class="phase4-checks"><label><input type="checkbox" name="safety"> Safety critical</label><label><input type="checkbox" name="comments"> Comments required</label><label><input type="checkbox" name="evidence"> Evidence required</label></div><button class="btn btn-secondary">Add criterion</button></form>`).join("")}<button class="btn publish-phase4-rubric" ${rubricCriteria.length?"":"disabled"}>Publish immutable rubric</button>`:""}</article>`}).join("");
  const rubricForm=published.length?`<form class="card setup-form" id="phase4RubricForm"><span class="eyebrow">PRACTICAL COMPETENCY BUILDER</span><h3>New assessment rubric</h3><label>Published pathway version<select name="versionId">${published.map(v=>`<option value="${escapeHtml(v.id)}">Version ${v.version_number}</option>`).join("")}</select></label><label>Rubric title<input name="title" required placeholder="Workplace practical assessment"></label><label>Assessor guidance<textarea name="assessorGuidance"></textarea></label><label>Worker guidance<textarea name="workerGuidance"></textarea></label><label class="checkbox-label"><input type="checkbox" name="workerAck"> Require worker acknowledgement</label><button class="btn">Create rubric</button><p class="auth-status"></p></form>`:"";
  return `${workspaceHero("COMPETENCY","Practical competency and evidence","Build criterion-level rubrics, capture workplace evidence, acknowledge outcomes, approve competency and preserve immutable history.")}<div class="stats-grid workspace-stats"><div class="stat-card"><span>Assignments</span><strong>${assignments.length}</strong></div><div class="stat-card"><span>Published rubrics</span><strong>${rubrics.filter(r=>r.status==="Published").length}</strong></div><div class="stat-card"><span>Awaiting approval</span><strong>${assessments.filter(a=>a.status==="Management Review").length}</strong></div><div class="stat-card"><span>Current competency</span><strong>${assignments.filter(a=>a.status==="Competent").length}</strong></div></div><section class="phase4-builder">${rubricForm}<div class="phase4-rubric-list">${rubricCards||'<section class="card purpose-empty"><h3>No practical rubric yet</h3><p>Create criterion-level assessment requirements for a published pathway.</p></section>'}</div></section><section class="phase3-layout">${form}<section class="phase3-register">${assignmentRows||'<section class="card purpose-empty"><h3>No assignments yet</h3><p>Create the first controlled assignment when staff access is ready.</p></section>'}</section></section>`;
}

function bindPhase3Competency(context) {
  bindSetupForm("phase3AssignmentForm",values=>authService.database.assignPublishedPathway({versionId:values.get("versionId"),workerUserId:values.get("workerUserId"),trainerUserId:values.get("trainerUserId")||null,dueAt:values.get("dueAt")?`${values.get("dueAt")}T23:59:59Z`:null}));
  document.querySelectorAll(".phase3-decision").forEach(button=>button.addEventListener("click",async()=>{const card=button.closest(".phase3-assignment"),status=button.dataset.decision;if(!confirm(`${status==="Competent"?"Approve competency":"Request reassessment"}? This decision is audited.`))return;button.disabled=true;try{await authService.database.decideCompetency(card.dataset.assignmentId,status,card.querySelector(".phase3-decision-notes")?.value||null);authenticatedContext=await authService.restore(state.activeOrganizationId);renderAuthenticatedWorkspace();}catch{button.disabled=false;alert("This competency decision is not authorised or evidence is incomplete.");}}));
  bindSetupForm("phase4RubricForm",values=>authService.database.createCompetencyRubric({versionId:values.get("versionId"),title:values.get("title"),assessorGuidance:values.get("assessorGuidance"),workerGuidance:values.get("workerGuidance"),workerAcknowledgementRequired:values.get("workerAck")==="on"}));
  document.querySelectorAll(".phase4-section-form").forEach(form=>form.addEventListener("submit",async event=>{event.preventDefault();await authService.database.addCompetencyRubricSection(form.closest(".phase4-rubric").dataset.rubric,new FormData(form).get("title"));authenticatedContext=await authService.restore(state.activeOrganizationId);renderAuthenticatedWorkspace();}));
  document.querySelectorAll(".phase4-criterion-form").forEach(form=>form.addEventListener("submit",async event=>{event.preventDefault();const values=new FormData(form);await authService.database.addCompetencyRubricCriterion({sectionId:form.dataset.section,criterion:values.get("criterion"),safetyCritical:values.get("safety")==="on",commentsRequired:values.get("comments")==="on",evidenceRequired:values.get("evidence")==="on"});authenticatedContext=await authService.restore(state.activeOrganizationId);renderAuthenticatedWorkspace();}));
  document.querySelectorAll(".publish-phase4-rubric").forEach(button=>button.addEventListener("click",async()=>{if(!confirm("Publish this rubric? Published criteria become immutable for evidence history."))return;await authService.database.publishCompetencyRubric(button.closest(".phase4-rubric").dataset.rubric);authenticatedContext=await authService.restore(state.activeOrganizationId);renderAuthenticatedWorkspace();}));
  document.querySelectorAll(".phase4-review").forEach(button=>button.addEventListener("click",async()=>{const card=button.closest(".phase3-assignment"),reason=card.querySelector(".phase4-review-reason").value.trim();if(reason.length<3)return alert("Record a review reason.");if(!confirm(`${button.dataset.decision}? The evidence decision and exact rubric version will be preserved.`))return;await authService.database.reviewCompetencyAssessment({assessmentId:button.dataset.assessment,decision:button.dataset.decision,reason,validityDays:Number(card.querySelector(".phase4-validity").value)||null});authenticatedContext=await authService.restore(state.activeOrganizationId);renderAuthenticatedWorkspace();}));
}

function renderPhase5Operations(context) {
  const role=context.membership.role, management=["Organisation Administrator","Facility Administrator","Department Manager"].includes(role);
  const tasks=context.workTasks||[], events=context.calendarEvents||[], notifications=context.userNotifications||[], announcements=context.announcements||[], receipts=new Set((context.announcementReceipts||[]).map(item=>item.announcement_id));
  const profiles=new Map((context.organizationStaff||[]).map(item=>[item.user_id,item.user_profiles||{}]));
  const memberships=context.organizationMemberships||[], versions=(context.learningPathwayVersions||[]).filter(item=>item.lifecycle==="Published"), pathways=context.learningPathways||[];
  const trainers=memberships.filter(item=>item.role?.includes("Trainer")), managers=memberships.filter(item=>["Organisation Administrator","Facility Administrator","Department Manager"].includes(item.role)), workers=memberships.filter(item=>["PCA","Cleaner","Support Worker"].includes(item.role));
  const personName=id=>escapeHtml(profiles.get(id)?.full_name||memberships.find(item=>item.user_id===id)?.role||"Assigned staff member");
  const now=Date.now(), openTasks=tasks.filter(item=>item.status==="Open"||item.status==="Blocked"), overdue=openTasks.filter(item=>item.due_at&&new Date(item.due_at).getTime()<now&&item.status==="Open");
  const taskCards=openTasks.map(item=>`<article class="phase5-task ${item.status==="Blocked"?"is-blocked":""}"><div><span class="eyebrow">${escapeHtml(item.task_type)} · ${escapeHtml(item.priority)}</span><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.detail||"Workflow action")}</p><small>${item.due_at?`${new Date(item.due_at).getTime()<now?"Overdue":"Due"} ${escapeHtml(new Date(item.due_at).toLocaleDateString("en-AU"))}`:"No due date"} · ${personName(item.assigned_to_user_id)}</small></div><span class="status-chip status-${item.status==="Open"?"warning":"neutral"}">${escapeHtml(item.status)}</span></article>`).join("");
  const eventCards=events.slice(0,30).map(item=>`<article class="phase5-event"><time datetime="${escapeHtml(item.starts_at)}"><strong>${escapeHtml(new Date(item.starts_at).toLocaleDateString("en-AU",{day:"numeric",month:"short"}))}</strong><small>${escapeHtml(new Date(item.starts_at).toLocaleTimeString("en-AU",{hour:"numeric",minute:"2-digit"}))}</small></time><div><span class="eyebrow">${escapeHtml(item.event_type)}</span><h3>${escapeHtml(item.title)}</h3></div></article>`).join("");
  const notificationCards=notifications.map(item=>`<article class="phase5-notification ${item.status==="Unread"?"is-unread":""}"><div><span class="eyebrow">${escapeHtml(item.notification_type)}</span><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.message)}</p><small>${escapeHtml(new Date(item.created_at).toLocaleString("en-AU"))}</small></div>${item.status==="Unread"?`<button class="link-button read-phase5-notification" data-notification="${escapeHtml(item.id)}">Mark read</button>`:'<span class="status-chip status-neutral">Read</span>'}</article>`).join("");
  const announcementCards=announcements.map(item=>`<article class="phase5-announcement"><div><span class="eyebrow">${escapeHtml(item.scope_kind)} · ${escapeHtml(item.priority)}</span><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.message)}</p><small>Published ${escapeHtml(new Date(item.published_at).toLocaleString("en-AU"))}</small></div>${receipts.has(item.id)?'<span class="status-chip status-success">Read</span>':`<button class="link-button read-phase5-announcement" data-announcement="${escapeHtml(item.id)}">Acknowledge</button>`}</article>`).join("");
  const preferences=(context.notificationPreferences||[])[0]||{digest:"Immediate",email_enabled:false,assignment_notifications:true,deadline_notifications:true,competency_notifications:true,announcement_notifications:true};
  const assignmentForm=management?`<form class="card phase5-form" id="phase5AssignmentForm"><span class="eyebrow">MULTI-LEVEL ASSIGNMENT</span><h3>Assign a published pathway</h3><div class="phase5-form-grid"><label>Published pathway<select name="versionId" required><option value="">Choose version</option>${versions.map(version=>`<option value="${escapeHtml(version.id)}">${escapeHtml(pathways.find(item=>item.id===version.pathway_id)?.title||"Pathway")} · Version ${version.version_number}</option>`).join("")}</select></label><label>Assignment title<input name="title" required placeholder="Required workforce pathway"></label><label>Scope<select name="scope" id="phase5Scope"><option>Individual</option><option>Selected Cohort</option><option>Department</option><option>Facility</option><option>Role Group</option></select></label><label>Role group<select name="roleGroup"><option value="">Not role-scoped</option><option>PCA</option><option>Cleaner</option><option>Support Worker</option></select></label><label>Facility<select name="facilityId"><option value="">Not facility-scoped</option>${(context.facilities||[]).map(item=>`<option value="${escapeHtml(item.id)}">${escapeHtml(item.name)}</option>`).join("")}</select></label><label>Department<select name="departmentId"><option value="">Not department-scoped</option>${(context.departmentDetails||[]).map(item=>`<option value="${escapeHtml(item.id)}">${escapeHtml(item.name)}</option>`).join("")}</select></label><label>Trainer<select name="trainerUserId"><option value="">Assign later</option>${trainers.map(item=>`<option value="${escapeHtml(item.user_id)}">${personName(item.user_id)}</option>`).join("")}</select></label><label>Manager<select name="managerUserId"><option value="">Assign later</option>${managers.map(item=>`<option value="${escapeHtml(item.user_id)}">${personName(item.user_id)}</option>`).join("")}</select></label><label>Priority<select name="priority"><option>Normal</option><option>High</option><option>Urgent</option><option>Low</option></select></label><label>Starts<input name="startsAt" type="datetime-local" required></label><label>Due<input name="dueAt" type="datetime-local"></label><label>Renewal interval days<input name="renewalDays" type="number" min="1" max="3650"></label></div>${workers.length?`<fieldset class="phase5-cohort"><legend>Individual or selected cohort</legend>${workers.map(item=>`<label><input type="checkbox" name="selectedUsers" value="${escapeHtml(item.user_id)}"> ${personName(item.user_id)} · ${escapeHtml(item.role)}</label>`).join("")}</fieldset>`:""}<button class="btn" type="submit" ${versions.length?"":"disabled"}>Create assignments</button>${versions.length?"":'<p class="form-guidance">Publish a pathway before assigning it.</p>'}<p class="auth-status" role="status"></p></form>`:"";
  const announcementForm=management?`<form class="card phase5-form" id="phase5AnnouncementForm"><span class="eyebrow">ANNOUNCEMENT</span><h3>Publish a scoped message</h3><label>Title<input name="title" required></label><label>Message<textarea name="message" minlength="3" required></textarea></label><div class="phase5-form-grid"><label>Scope<select name="scope"><option>Organisation</option><option>Facility</option><option>Department</option><option>Role</option></select></label><label>Priority<select name="priority"><option>Normal</option><option>High</option><option>Urgent</option><option>Low</option></select></label><label>Facility<select name="facilityId"><option value="">Not facility-scoped</option>${(context.facilities||[]).map(item=>`<option value="${escapeHtml(item.id)}">${escapeHtml(item.name)}</option>`).join("")}</select></label><label>Department<select name="departmentId"><option value="">Not department-scoped</option>${(context.departmentDetails||[]).map(item=>`<option value="${escapeHtml(item.id)}">${escapeHtml(item.name)}</option>`).join("")}</select></label><label>Role<select name="roleGroup"><option value="">Not role-scoped</option>${["PCA","Cleaner","Support Worker","PCA Trainer","Cleaner Trainer","Department Manager"].map(item=>`<option>${item}</option>`).join("")}</select></label><label>Expires<input name="expiresAt" type="datetime-local"></label></div><button class="btn" type="submit">Publish announcement</button><p class="auth-status" role="status"></p></form>`:"";
  const preferenceForm=`<form class="card phase5-form" id="phase5PreferencesForm"><span class="eyebrow">NOTIFICATION PREFERENCES</span><h3>Choose how updates reach you</h3><label>Delivery timing<select name="digest">${["Immediate","Daily Digest","Weekly Digest","In-App Only"].map(item=>`<option ${preferences.digest===item?"selected":""}>${item}</option>`).join("")}</select></label><label class="checkbox-label"><input type="checkbox" name="email" ${preferences.email_enabled?"checked":""}> Queue email when an approved provider is configured</label>${[["assignments","Assignment notifications",preferences.assignment_notifications],["deadlines","Deadline and overdue notifications",preferences.deadline_notifications],["competency","Competency notifications",preferences.competency_notifications],["announcements","Announcement notifications",preferences.announcement_notifications]].map(([name,label,checked])=>`<label class="checkbox-label"><input type="checkbox" name="${name}" ${checked!==false?"checked":""}> ${label}</label>`).join("")}<p class="trust-note">In-app notifications are active. Email delivery is never marked delivered until a configured provider confirms it.</p><button class="btn" type="submit">Save preferences</button><p class="auth-status" role="status"></p></form>`;
  renderShell(`${organizationSwitcher(context)}<section class="dashboard-hero workspace-page-hero"><div class="dashboard-welcome"><span class="eyebrow">ASSIGNMENTS · CALENDAR · TO-DO</span><h2>Work and notifications</h2><p>See the next authorised action, every important date and organisation messages in one role-aware workspace.</p></div>${management?'<button class="btn btn-secondary" id="phase5RefreshDeadlines">Refresh deadline status</button>':""}</section><div class="stats-grid workspace-stats"><div class="stat-card"><span>Open work</span><strong>${openTasks.filter(item=>item.status==="Open").length}</strong></div><div class="stat-card"><span>Overdue</span><strong>${overdue.length}</strong></div><div class="stat-card"><span>Upcoming dates</span><strong>${events.filter(item=>new Date(item.starts_at).getTime()>=now).length}</strong></div><div class="stat-card"><span>Unread</span><strong>${notifications.filter(item=>item.status==="Unread").length}</strong></div></div>${assignmentForm}<section class="phase5-operations-grid"><section class="card"><div class="section-heading"><div><span class="eyebrow">ROLE-AWARE TO-DO</span><h3>Actions requiring attention</h3></div></div><div class="phase5-list">${taskCards||'<p class="empty-state">No open work is assigned to this view.</p>'}</div></section><section class="card"><div class="section-heading"><div><span class="eyebrow">CALENDAR</span><h3>Training and competency dates</h3></div></div><div class="phase5-list">${eventCards||'<p class="empty-state">No scheduled dates are visible yet.</p>'}</div></section></section><section class="phase5-operations-grid"><section class="card"><div class="section-heading"><div><span class="eyebrow">NOTIFICATIONS</span><h3>Your updates</h3></div></div><div class="phase5-list">${notificationCards||'<p class="empty-state">You have no notifications.</p>'}</div></section><section class="card"><div class="section-heading"><div><span class="eyebrow">ANNOUNCEMENTS</span><h3>Organisation messages</h3></div></div><div class="phase5-list">${announcementCards||'<p class="empty-state">No active announcements.</p>'}</div></section></section><section class="phase5-settings-grid">${announcementForm}${preferenceForm}</section>`);
  bindAuthenticatedWorkspace(context);
  bindPhase5Operations(context);
}

function bindPhase5Operations(context) {
  bindSetupForm("phase5AssignmentForm",values=>authService.database.createAssignmentBatch({versionId:values.get("versionId"),title:values.get("title"),scope:values.get("scope"),selectedUsers:values.getAll("selectedUsers"),facilityId:values.get("facilityId"),departmentId:values.get("departmentId"),roleGroup:values.get("roleGroup"),trainerUserId:values.get("trainerUserId"),managerUserId:values.get("managerUserId"),priority:values.get("priority"),startsAt:new Date(values.get("startsAt")).toISOString(),dueAt:values.get("dueAt")?new Date(values.get("dueAt")).toISOString():null,renewalRule:values.get("renewalDays")?{intervalDays:Number(values.get("renewalDays"))}:{}}));
  bindSetupForm("phase5AnnouncementForm",values=>authService.database.publishAnnouncement(context.organization.id,{title:values.get("title"),message:values.get("message"),scope:values.get("scope"),facilityId:values.get("facilityId"),departmentId:values.get("departmentId"),roleGroup:values.get("roleGroup"),priority:values.get("priority"),expiresAt:values.get("expiresAt")?new Date(values.get("expiresAt")).toISOString():null}));
  bindSetupForm("phase5PreferencesForm",values=>authService.database.saveNotificationPreferences(context.organization.id,{digest:values.get("digest"),emailEnabled:values.get("email")==="on",assignmentNotifications:values.get("assignments")==="on",deadlineNotifications:values.get("deadlines")==="on",competencyNotifications:values.get("competency")==="on",announcementNotifications:values.get("announcements")==="on"}));
...
function renderOrganizationAdministration(context) {
  ...
  if (view === "home") {
    ...
    content = pathwayAuthoringHtml(context, workspaceHero);
  } else if (view === "people") {
    ...
    content = phase3CompetencyHtml(context, workspaceHero);
