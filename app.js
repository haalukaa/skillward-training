...
function renderAuthenticatedWorkspace() {
  ...
  const phase3Cards = phase3Assignments.map(assignment => {
    ...
    const trainerActions=trainer?`<label>Observed evidence<textarea class="phase3-observation-note" data-assignment="${escapeHtml(assignment.id)}"></textarea></label><label>Outcome<select class="phase3-observation-outcome" data-assignment="${escapeHtml(assignment.id)}"><option>Competent</option><option>Reassessment Required</option></select></label><div class="profile-actions"><button class="btn record-phase3-observation" data-assignment="${escapeHtml(assignment.id)}">Record observation</button><button class="btn submit-phase3-recommendation" data-assignment="${escapeHtml(assignment.id)}">Send recommendation</button></div>`:"";
    return `<article class="card phase3-learning-card"><span class="eyebrow">VERSION ${escapeHtml(version?.version_number||"")} · ${escapeHtml(assignment.status)}</span><h3>${escapeHtml(pathway?.title||"Assigned pathway")}</h3><div class="area-progress"><span style="width:${Number(assignment.progress_percent)||0}%"></span></div><small>${Number(assignment.progress_percent)||0}% complete</small>${workerItems}${trainerActions}<p class="auth-status" id="phase3-status-${escapeHtml(assignment.id)}"></p></article>`;
  }).join("");
...
function phase3CompetencyHtml(context, workspaceHero) {
  ...
  const assignmentRows=assignments.map(item=>{const worker=profiles.get(item.worker_user_id)||{};const version=published.find(v=>v.id===item.pathway_version_id);const pathway=(context.learningPathways||[]).find(p=>p.id===item.pathway_id);const observation=observations.find(o=>o.assignment_id===item.id);const recommendation=recommendations.find(o=>o.assignment_id===item.id);const award=awards.find(o=>o.assignment_id===item.id);return `<article class="phase3-assignment" data-assignment-id="${escapeHtml(item.id)}"><div><span class="eyebrow">${escapeHtml(item.status)}</span><h3>${escapeHtml(worker.full_name||"Assigned worker")}</h3><p>${escapeHtml(pathway?.title||"Published pathway")} · Version ${escapeHtml(version?.version_number||"")}</p><div class="area-progress"><span style="width:${Number(item.progress_percent)||0}%"></span></div><small>${Number(item.progress_percent)||0}% learning complete${item.due_at?` · Due ${escapeHtml(new Date(item.due_at).toLocaleDateString("en-AU"))}`:""}</small></div><div class="phase3-evidence"><span>Observation: <strong>${observation?escapeHtml(observation.outcome):"Pending"}</strong></span><span>Recommendation: <strong>${recommendation?escapeHtml(recommendation.recommendation):"Pending"}</strong></span><span>Decision: <strong>${award?escapeHtml(award.decision):"Pending"}</strong></span>${item.status==="Sent to Management"?`<label>Decision notes<textarea class="phase3-decision-notes"></textarea></label><div><button class="btn phase3-decision" data-decision="Competent">Approve competency</button><button class="btn btn-secondary phase3-decision" data-decision="Reassessment Required">Request reassessment</button></div>`:""}</div></article>`}).join("");
  ...
  return `${workspaceHero("COMPETENCY","Assignments and competency","Assign published learning, validate knowledge, record practical evidence, approve competency and monitor renewal.")}<div class="stats-grid workspace-stats"><div class="stat-card"><span>Assignments</span><strong>${assignments.length}</strong></div><div class="stat-card"><span>Ready for trainer</span><strong>${assignments.filter(a=>a.status==="Ready for Trainer").length}</strong></div><div class="stat-card"><span>Awaiting approval</span><strong>${assignments.filter(a=>a.status==="Sent to Management").length}</strong></div><div class="stat-card"><span>Current competency</span><strong>${awards.filter(a=>a.decision==="Competent").length}</strong></div></div><section class="phase3-layout">${form}<section class="phase3-register">${assignmentRows||'<section class="card purpose-empty"><h3>No assignments yet</h3><p>Create the first controlled assignment when staff access is ready.</p></section>'}</section></section>`;
}
