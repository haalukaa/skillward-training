(() => {
  ...
  function openDialog(action, context = {}) {
    ...
    const definitions = {
      ...
      transition_organization: { title: `Change ${context.name || "organisation"} status`, impact: "Suspension never deletes data; it immediately blocks active memberships. Archived and offboarding stages remain recoverable and audited.", fields: `<input type="hidden" name="organization_id" value="${esc(context.organization_id)}"><label>New lifecycle status<select name="status"><option>setup</option><option>pilot</option><option>active</option><option>grace_period</option><option>suspended</option><option>archived</option><option>offboarded</option></select></label>` },
