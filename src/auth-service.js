...
export class AuthService {
  ...
  async resolve(user, organizationId = null) {
    const context = await this.database.loadSessionContext(user, organizationId);
    const status = context.profile.account_status !== "Active" ? context.profile.account_status : (context.membership.membership_status || context.membership.account_status);
  ...
  async restore(organizationId = null) { if (!this.client) return null; const { data } = await this.client.auth.getUser(); return data.user ? this.resolve(data.user, organizationId) : null; }
  async switchOrganization(organizationId) { if (!this.client) throw new Error("CONFIGURATION_MISSING"); const { data } = await this.client.auth.getUser(); if (!data.user) throw new Error("INVALID_CREDENTIALS"); return this.resolve(data.user, organizationId); }
