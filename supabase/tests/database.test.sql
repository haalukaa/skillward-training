...
reset role; set local role postgres; select set_config('request.jwt.claim.sub','',true); select set_config('request.jwt.claim.role','',true); select pg_temp.as_user('10000000-0000-0000-0000-000000000005'); select is((select count(*)::int from public.trainer_assignments),1,'PCA Trainer sees assigned PCA only'); select is((select count(*)::int from public.departments),1,'PCA Trainer sees assigned departments only'); select is((with attempted as (update public.signoff_recommendations set management_decision='Approved' where id='a1200000-0000-0000-0000-000000000001' returning 1) select count(*)::int from attempted),0,'trainer cannot final approve');
...
select has_index('public','departments','departments_hospital_idx','hospital index exists'); select has_index('public','hospital_memberships','hm_user_idx','membership index exists'); select has_trigger('public','trainer_assignments','validate_trainer_assignment','trainer validation trigger exists'); select has_trigger('public','audit_logs','audit_append_only','append-only trigger exists'); select has_function('public','has_hospital_role',array['uuid','workplace_role[]'],'safe role helper exists'); select ok(has_function_privilege('service_role','public.mark_expired_competencies(date)','EXECUTE') and not has_function_privilege('authenticated','public.mark_expired_competencies(date)','EXECUTE') and not has_function_privilege('anon','public.mark_expired_competencies(date)','EXECUTE'),'expiry function executable only by service_role'); select isnt((select prosecdef from pg_proc where oid='public.touch_updated_at()'::regprocedure),true,'timestamp trigger is not security definer');
